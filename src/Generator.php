...
final class Generator implements Domain\Generator
{
     ...
     */
    private $signer;
    public function __construct($clientEmail, $privateKey, Signer $signer = null)
    {
    ...
    public function createCustomToken($uid, array $claims = [], \DateTimeInterface $expiresAt = null)
    {
        ...
        return $this->builder->setIssuedAt($now)->setExpiration($expiration)->sign($this->signer, $this->privateKey)->getToken();
    }
    private function createBuilder()
    {
        return (new Builder())->setIssuer($this->clientEmail)->setSubject($this->clientEmail)->setAudience('https://identitytoolkit.googleapis.com/google.identity.identitytoolkit.v1.IdentityToolkit');
    }
}